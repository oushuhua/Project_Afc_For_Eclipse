 ...
 * @date 2014-12-24 下午2:47:40
 */
public class ViewFlow extends AdapterView<Adapter> {

	public List<ADEntity.Detail> adList = new ArrayList<ADEntity.Detail>();
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (xDiff < 3 && adList.size()>0) {
				ADEntity.Detail cmd = adList.get(getSelectedItemPosition() % adList.size());
				AdHandler.handleIt(cmd, getContext());
	...
	public void setSelection(int position) {
		...
		for (int offset = 1; mSideBuffer - offset >= 0; offset++) {
			...
			if (leftIndex >= 0)
				mLoadedViews.addFirst(
						makeAndAddView(leftIndex, false, (recycleViews.isEmpty() ? null : recycleViews.remove(0))));
			if (rightIndex < mAdapter.getCount())
				mLoadedViews.addLast(
						makeAndAddView(rightIndex, true, (recycleViews.isEmpty() ? null : recycleViews.remove(0))));
		}
	...
	private View setupChild(View child, boolean addToEnd, boolean recycle) {
		...
		if (p == null) {
			p = new AbsListView.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT,
					0);
		}
		if (recycle)
